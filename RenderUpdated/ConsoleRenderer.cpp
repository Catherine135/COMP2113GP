...
void ConsoleRenderer::render(const RenderTask& task) {
    ...
    } else if (const auto* t = dynamic_cast<const NameInterface*>(&task)){
        renderNameInterface(*t);
...
void ConsoleRenderer::renderStartInterface (const StartInterface& task){
    ...
    moveCursor(13,23);
    std::cout << WHITE_BG << RED << " Welcome to generAIs.io, a human vs AI board game!! ";
    moveCursor(17,37);
    std::cout << WHITE_BG << BLUE << " START        (PRESS 1) ";
    moveCursor(19,37);
    std::cout << WHITE_BG << BLUE << " LEADERBOARD  (PRESS 2) ";
    moveCursor(21,37);
    std::cout << WHITE_BG << BLUE << " EXIT         (PRESS 3) " << DFT;

    moveCursor(40,0);
    std::cout.flush();
}

/// @brief allows the user to set their username for the game
///             further UI integration!!!!!!
/// @param task the NameInterface task
void ConsoleRenderer::renderNameInterface(const NameInterface& task){
    std::string name;
    std::cout << CLEAR;
    moveCursor(3,10);
    std::cout << " ██████╗ ███████╗███╗   ██╗███████╗██████╗  █████╗ ██╗███████╗   ██╗ ██████╗ ";
    ...
    moveCursor(13,28);
    std::cout << WHITE_BG << RED << " Please enter your name: ";
    moveCursor(13,55);
    std::getline(std::cin, name);
    moveCursor(17,28);
    std::cout << WHITE_BG << BLUE << " Welcome, " << name << "! " << DFT;
...
void ConsoleRenderer::renderDifficultyInterface (const DifficultyInterface& task){
    ...
    moveCursor(13,28);
    std::cout << WHITE_BG << RED << " Please choose the difficulty level: ";
    moveCursor(17,37);
    std::cout << WHITE_BG << BLUE << " EASY    (PRESS 1) ";
    moveCursor(19,37);
    std::cout << WHITE_BG << BLUE << " MEDIUM  (PRESS 2) ";
    moveCursor(21,37);
    std::cout << WHITE_BG << BLUE << " HARD    (PRESS 3) " << DFT;
...
void ConsoleRenderer::renderLeaderboardInterface(const LeaderboardInterface& task){
    ...
    moveCursor(12,6);
    std::cout << WHITE_BG << BLUE << "EASY" << DFT;
    moveCursor(13,6);
    std::cout << "-----------------------------------------------------------------------------";
    ...
    moveCursor(18,6);
    std::cout << WHITE_BG << BLUE << "MEDIUM" << DFT;
    moveCursor(19,6);
    std::cout << "-----------------------------------------------------------------------------";
    ...
    moveCursor(24,6);
    std::cout << WHITE_BG << BLUE << "HARD" << DFT;
    moveCursor(25,6);
    std::cout << "-----------------------------------------------------------------------------";
    std::ifstream fin3(task.fileNameHard);
    for (int i = 0; i < 3; i++){
    ...
    moveCursor(30,6);
    std::cout << WHITE_BG << RED << "Press \"E\" to exit" << DFT;
